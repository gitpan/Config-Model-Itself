...
specify where to insert accepted element. This will change the behavior but will help generating more consistent  user interfaces. I<< Optional. Type reference.  >>
...
Raise an error if the test code snippet does returns false. Note this snippet will also be run on undefined value, which may not be what you want.I<< Optional. Type hash of node.  >>

=head2 warn_unless

Warn user if the code snippet returns false. I<< Optional. Type hash of node.  >>

=head2 warn_if_match

Warn user if a I<defined> value matches the regular expression. I<< Optional. Type hash of node.  >>

=head2 warn_unless_match

Warn user if I<defined> value does not match the regular expression. I<< Optional. Type hash of node.  >>
