...
Note: backend is migrated with 'C<$old>' and with $old => "C<- syntax>" and 'C<$replace{cds}>' => "C<cds_file>", 'C<$replace{perl}>' => "C<perl_file>", 'C<$replace{ini}>' => "C<ini_file>"
...
Note: file is migrated with 'C<$old>' and with $old => "C<- config_file>"
...
Specify the configuration file (without path) that will store configuration information. B<Deprecated> I<< Optional. Type uniline.  >>
...
Note: config_dir is migrated with 'C<$old>' and with $old => "C<- - write_config_dir>"
